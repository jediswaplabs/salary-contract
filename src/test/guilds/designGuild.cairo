...
//
#[starknet::interface]
trait IDesignGuildSBT<TContractState> {
    // view functions
...
mod DesignGuildSBT {
    ...
    #[external(v0)]
    impl DesignGuildSBT of super::IDesignGuildSBT<ContractState> {
        //
        // Getters
        //
        fn tokenURI(self: @ContractState, token_id: u256) -> Span<felt252> {
            ...
            // @notice this is a sample SBT contract for design guild, update the next line before deploying other guild
            let points = masterDispatcher.get_guild_points(owner, 'design');
        ...
        fn tokenURI_from_contributor(self: @ContractState, contributor: ContractAddress) -> Span<felt252> {
            ...
            let masterDispatcher = IMasterDispatcher { contract_address: master };
            // @notice this is a sample SBT contract for design guild, update the next line before deploying other guild
            let points = masterDispatcher.get_guild_points(contributor, 'design');
        ...
        fn safe_mint(ref self: ContractState, token_type: u8) {
            ...
            let master = self._master.read();
            let masterDispatcher = IMasterDispatcher { contract_address: master };
            let points = masterDispatcher.get_guild_points(account, 'design');
            let tier = InternalImpl::_get_contribution_tier(@self, points);
