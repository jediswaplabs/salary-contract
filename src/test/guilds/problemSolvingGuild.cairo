...
//
#[starknet::interface]
trait IProblemSolvingGuildSBT<TContractState> {
    // view functions
...
mod ProblemSolvingGuildSBT {
    ...
    use salary::utils::storage::StoreSpanFelt252;
    use salary::utils::array::StoreU32Array;
    use super::{
    ...
    #[external(v0)]
    impl ProblemSolvingGuildSBT of super::IProblemSolvingGuildSBT<ContractState> {
        //
        // Getters
        //
        fn tokenURI(self: @ContractState, token_id: u256) -> Span<felt252> {
            ...
            // @notice this is a sample SBT contract for problem_solving guild, update the next line before deploying other guild
            let points = masterDispatcher.get_guild_points(owner, 'problem_solving');
        ...
        fn tokenURI_from_contributor(self: @ContractState, contributor: ContractAddress) -> Span<felt252> {
            ...
            let masterDispatcher = IMasterDispatcher { contract_address: master };
            // @notice this is a sample SBT contract for problem_solving guild, update the next line before deploying other guild
            let points = masterDispatcher.get_guild_points(contributor, 'problem_solving');
        ...
        fn safe_mint(ref self: ContractState, token_type: u8) {
            ...
            let master = self._master.read();
            let masterDispatcher = IMasterDispatcher { contract_address: master };
            let points = masterDispatcher.get_guild_points(account, 'problem_solving');
            let tier = InternalImpl::_get_contribution_tier(@self, points);
